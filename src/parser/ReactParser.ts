...
export class ReactParser implements IComponentParser {
  ...
  async parseComponentFile(filePath: string): Promise<ComponentMeta | null> {
    ...
    console.log("[2.0] Parsing AST for React file:", filePath);
    ...
    const usesChildren = this.detectChildrenUsage(source, compName, text);
    if (usesChildren) {
